  ...
  <section class="event-all" :class="{'event-all-show': isShow}">
    <div class="table-box">
      <div class="edit-input" :class="{'edit-input-show': is_edit}">
        ...
        <button @click="editData">确认</button>
      </div>
      <div class="screen-box">
        ...
        <input type="text" class="div-search" v-model="screen_title" placeholder="筛选关键词" @keyup.esc="clearInput">
      </div>
...
<script>
  export default {
    ...
    computed: {
      notepad () {
        const self = this // 原版这里有错**** , 底下可以简化。
        return self.$store.getters.getEventList.filter((d) => { // 这里就是过滤框的几种情况
          if (self.screen_type !== 0 && self.screen_title === '') {
            return d.type === self.screen_type
          } else if (self.screen_type !== 0 && self.screen_title !== '') {
            return d.type === self.screen_type && d.content.indexOf(self.screen_title !== -1)
          } else if (self.screen_type === 0 && self.scr !== '') {
            return d.content.indexOf(self.screen_title) !== -1
          } else {
            return true
          }
        })
